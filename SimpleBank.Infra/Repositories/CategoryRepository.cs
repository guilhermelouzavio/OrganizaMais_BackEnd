...
namespace SimpleBank.Infra.Repositories
{
    public class CategoryRepository : ICategoryRepository
    {
        ...
        {
            var sql = "INSERT INTO Categories (UserId, Name, Type, CreatedAt) VALUES (@UserId, @Name, @Type::text, @CreatedAt) RETURNING Id";
            using var db = Connection;
            // Para enums, o Dapper precisa de um mapeamento ou que você converta para string/int
            category.Id = await db.ExecuteScalarAsync<int>(sql, new { category.UserId, category.Name, Type = category.Type.ToString(), category.CreatedAt });
        }
        ...
        {
            var sql = "SELECT Id, UserId, Name, Type, CreatedAt FROM Categories WHERE Id = @Id";
            using var db = Connection;
        ...
        {
            var sql = "SELECT Id, UserId, Name, Type, CreatedAt FROM Categories WHERE UserId = @UserId";
            using var db = Connection;
        ...
        {
            var sql = "SELECT Id, UserId, Name, Type, CreatedAt FROM Categories WHERE UserId IS NULL";
            using var db = Connection;
